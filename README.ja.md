...
**f4next** (Fetch For Next) は、**Next.js 13+ App Router** 向けに特別に設計された、軽量で型安全な `fetch` ラッパーです。
...
Axios のように `f4next.get`、`post`、`put`、`delete` を使用しますが、内部ではネイティブの Fetch API を使用します。
